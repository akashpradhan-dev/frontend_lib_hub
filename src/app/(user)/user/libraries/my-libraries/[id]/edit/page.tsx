...
import { useUpdateLibraryMutation } from '@/services/mutation/user/updateLibrary'
...
export default function EditLibrary() {
  ...
  const { data, status: libraryStatus } = useMyLibraryByIdQuery({
    id: libraryId?.toString() || '',
  })

  const libraryDetails = data?.data

  const { mutate, status } = useUpdateLibraryMutation()
  ...
  const getDefaultValues = () => {
    if (libraryDetails) {
      return {
        ...
        repositoryUrl: libraryDetails.repositoryUrl || '',
        homepageUrl: libraryDetails.homepageUrl || '',
        exampleUsage: libraryDetails.exampleUsage || '',
        category: libraryDetails.category || '',
        language: libraryDetails.language || '',
        framework: libraryDetails.framework || '',
        libraryType: libraryDetails.libraryType || '',
      }
    }
    // Return the initial empty state while loading
    return initForm
  }
  ...
  const onSubmit = (data: LibraryFormData) => {
    const payload = { ...data, id: libraryId?.toString() || '' }
    mutate(payload, {
      onSuccess: () => {
        form.reset()
  ...
  return (
    <section className="flex flex-col min-h-[90vh] items-center justify-center sm:px-6 lg:px-8">
      <div className="w-full  max-w-4xl">
        <Button
          variant="link"
          onClick={() => router.back()}
          className="mb-6 cursor-pointer justify-start gap-2 text-sm text-muted-foreground hover:text-foreground"
        >
          <ChevronLeft className="h-4 w-4" />
          Back to Libraries
        </Button>
      </div>
      <div className="w-full max-w-4xl rounded-2xl bg-white/70 dark:bg-black/70 backdrop-blur-xl p-8 shadow-xl border border-slate-200 dark:border-slate-800">
        {libraryStatus === 'pending' ? (
          <Spinner className="size-10" />
        ) : (
          <>
            <h2 className="text-3xl font-bold text-center mb-8 bg-gradient-color bg-clip-text text-transparent">
              Edit your library
            </h2>
